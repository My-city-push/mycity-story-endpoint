...
async function uploadRemoteToCloudinary(sourceUrl) {
...
async function run() {
  if (!/^true$/i.test(String(process.env.RUN_CLOUDINARY_REMOTE_STORY_TEST || "false"))) return;
  ...
  const sourceUrl = String(process.env.CLOUDINARY_REMOTE_SOURCE_URL || "").trim();
  if (!/^https:\/\//i.test(sourceUrl)) throw new Error("CLOUDINARY_REMOTE_SOURCE_URL missing");
  ...
  const requestId = "localdesk-la-fiesta-zoo-cloudinary-remote-v1";
  const reqRef = db.ref("aiStoryRequests/" + requestId);
  const prior = await reqRef.get();
  if (prior.exists() && prior.val()?.storyId) {
    console.log("CLOUDINARY_REMOTE_STORY_TEST duplicate storyId=" + prior.val().storyId);
    return;
  }

  const uploaded = await uploadRemoteToCloudinary(sourceUrl);
  ...
  console.log("CLOUDINARY_REMOTE_STORY_TEST created storyId=" + storyRef.key + " secureUrl=" + mediaUrl + " size=" + (uploaded.width || 0) + "x" + (uploaded.height || 0));
}

try {
  await run();
} catch (error) {
  console.error("CLOUDINARY_REMOTE_STORY_TEST failed:", error?.message || error);
}
